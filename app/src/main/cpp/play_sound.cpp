...
int FMOD_Main()
{
    ...
    */
    ...
    result = system->createSound(Common_MediaPath("640148main_APU Shutdown.ogg"), FMOD_DEFAULT, 0, &sound1);
    ERRCHECK(result);

    result = sound1->setMode(FMOD_LOOP_OFF);    /* drumloop.wav has embedded loop points which automatically makes looping turn on, */
    ERRCHECK(result);                           /* so turn it off here.  We could have also just put FMOD_LOOP_OFF in the above CreateSound call. */

    result = system->createSound(Common_MediaPath("Kalimba.mp3"), FMOD_DEFAULT, 0, &sound2);
    ERRCHECK(result);

    result = system->createSound(Common_MediaPath("640166main_MECO.ogg"), FMOD_DEFAULT, 0, &sound3);
    ERRCHECK(result);
